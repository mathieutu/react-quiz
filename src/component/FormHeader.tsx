...
function PreviousButton({ disabled, onClick }: { onClick: () => void, disabled: boolean }) {
  ...
  return (
    <span className="shadow-sm rounded-md">
      ...
      >
        <LoadingIcon loading={disabled} className="-ml-1 mr-2 h-5 w-5 text-gray-500">
...
type NextButtonProps = { onClick: () => void, disabled: boolean, isLastQuestion: boolean }

const NextButton = ({ disabled, onClick, isLastQuestion }: NextButtonProps) => (
  <span className="shadow-sm rounded-md">
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <LoadingIcon loading={disabled} className="-mr-1 ml-2 h-5 w-5">
  ...
  return (
    <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 md:flex md:items-center md:justify-between">
      ...
      <div className="mt-5 flex flex-wrap gap-3 lg:mt-0 lg:ml-4">
        <PreviousButton onClick={onPrevious} disabled={loading} />
        <NextButton onClick={onNext} disabled={loading} isLastQuestion={questionsLeftQuantity === 1} />
      </div>
    </header>
  )
}
