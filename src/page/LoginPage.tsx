    ...
    <div className="text-lg flex h-screen bg-white w-screen">
      <form onSubmit={handleSubmit} className="m-auto w-100 shadow-lg bg-gray-300 flex flex-col p-6">
        ...
        <div className="my-4 w-full">
          ...
          <input type="email" required className="w-full focus:shadow-lg p-2 px-3 rounded"
                 placeholder="exemple@domaine.com" onChange={(e) => setEmail(e.currentTarget.value)} />
        </div>
        <div className="my-4 w-full">
          <label className="mb-2">Nom et Prénom <span className="text-red-800">*</span></label>
          <input type="text" required className="w-full focus:shadow-lg p-2 px-3 rounded" placeholder="Jean Dupont"
                 onChange={(e) => setName(e.currentTarget.value)} />
        </div>
        <div className="mt-8 flex">
          <Button text={loading ? 'Chargement' : 'Commencer'}
                  icon={loading ? <CgSpinnerTwoAlt className="ml-2 animate-spin" /> :
